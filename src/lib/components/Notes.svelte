<script lang="ts">
	...
	function insertTab() {
		...
		let line2 = document.createTextNode(
			'b|-----------------|-----------------|-----------------|-----------------|'
		);
		let line3 = document.createTextNode(
			'g|-----------------|-----------------|-----------------|-----------------|'
		);
		let line4 = document.createTextNode(
			'd|-----------------|-----------------|-----------------|-----------------|'
		);
		let line5 = document.createTextNode(
			'a|-----------------|-----------------|-----------------|-----------------|'
		);
		let line6 = document.createTextNode(
			'e|-----------------|-----------------|-----------------|-----------------|'
		);
		...
		wrapper.appendChild(document.createElement('br'));
		wrapper.appendChild(line5);
		wrapper.appendChild(document.createElement('br'));
		wrapper.appendChild(line6);
...
<div class="flex w-[400px] flex-col">
	{#if playlistState.selectedPlaylist}
		<div class="flex-1 space-y-2 overflow-auto">
			{#if playlistState.selectedPlaylist}
				{#each playlistState.selectedPlaylist.notes as note}
		...
		<div class="mt-2 flex flex-col space-y-2">
			<div class="flex flex-col rounded-xl border-1 border-zinc-800">
				<div class="flex flex-row border-b-1 border-b-zinc-800">
					<!-- Basic -->
					<button
						class="flex cursor-pointer items-center rounded-tl-xl border-r-1 border-r-zinc-700 bg-zinc-800 p-1 hover:bg-zinc-700"
						onclick={bold}
					>
